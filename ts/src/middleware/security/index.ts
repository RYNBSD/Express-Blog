...
export const security = {
    async csrf(req: Request, _res: Response, next: NextFunction) {
        ...
        if (token.length === 0) throw new Error("Empty CSRF token");

        if (token instanceof Array) throw new Error("Too many CSRF token");

        const secret = req.session.csrf?.secret ?? "";
        if (secret.length === 0) throw new Error("Empty CSRF secret");
